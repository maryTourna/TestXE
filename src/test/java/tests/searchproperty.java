...
// properties of the search results. When searching for Παγκράτι (or other areas for that matter) a
// popup may appear that asks you to select one specific area of those proposed - in our case we
// want to select all areas proposed by autocomplete before proceeding to a search.
...
public class searchproperty extends BaseTest {
  ...
  @Test
  @Description(
      "This test attempts to find all region compatible with pagrati  . Fails if any error happens.")
  @Epic("Landing Page")
  public void testLandingPage() {
    ...
    try {
      WebElement button =
          wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//span[text()='ΣΥΜΦΩΝΩ']")));
      button.click();
  ...
  public void verifyRentIsSelected() {
    ...
    WebElement rentButton =
        wait.until(
            ExpectedConditions.elementToBeClickable(By.cssSelector("button[data-testid='rent']")));
    rentButton.click();
    ...
    if (rentClass != null) {
      Assert.assertTrue(
          rentClass.contains("selected"), "Το κουμπί 'Ενοικίαση' δεν είναι επιλεγμένο!");
    }
  }
}
